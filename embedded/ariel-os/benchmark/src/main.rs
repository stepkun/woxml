...
fn create_xml_bytes(
    writer: &mut XmlWriter<'_, bytes::BytesMut>,
    nsmap: &Vec<(Option<&'static str>, &'static str)>,
) {
    _ = writer.begin_elem("OTDS");
    _ = writer.ns_decl(nsmap);
...
fn main() {
    ...
    match ariel_os::bench::benchmark(100, || {
        ...
        Ok(ticks) => {
            info!("compact with Vec took {} per iteration", ticks);
        }
        Err(err) => {
    ...
    match ariel_os::bench::benchmark(100, || {
        let mut xml = XmlWriter::compact_mode(bytes::BytesMut::new());
        create_xml_bytes(&mut xml, &nsmap);
    }) {
        Ok(ticks) => {
            info!("compact with BytesMut took {} per iteration", ticks);
        }
        Err(err) => {
            error!("benchmark returned: {}", err);
            exit(ExitCode::FAILURE);
    ...
    match ariel_os::bench::benchmark(100, || {
        ...
        Ok(ticks) => {
            info!("pretty with Vec took {} per iteration", ticks);
        }
        Err(err) => {
            error!("benchmark returned: {}", err);
            exit(ExitCode::FAILURE);
    ...
    match ariel_os::bench::benchmark(100, || {
        let mut xml = XmlWriter::pretty_mode(bytes::BytesMut::new());
        create_xml_bytes(&mut xml, &nsmap);
    }) {
        Ok(ticks) => {
            info!("compact with BytesMut took {} per iteration", ticks);
