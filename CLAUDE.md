...
- [tests/woxml.rs](tests/woxml.rs) — 34 tests covering `XmlWriter` API (elements, attributes, namespaces, escaping, pretty/compact modes)
- [tests/write.rs](tests/write.rs) — 4 tests covering the `Write` trait's default `write_all` implementation (chunked writes, stall/error propagation, empty buffer)

## Coverage Notes

The `write_all` default method in [src/write.rs](src/write.rs) carries `#[cfg_attr(coverage_nightly, coverage(off))]` to suppress an LLVM phantom instantiation (`<_ as Write>::write_all`) that would otherwise show 0-count regions and skew the summary to 75%. The feature gate `cfg_attr(coverage_nightly, feature(coverage_attribute))` is declared in [src/lib.rs](src/lib.rs). Both activate only when cargo-llvm-cov sets the `coverage_nightly` cfg flag.
